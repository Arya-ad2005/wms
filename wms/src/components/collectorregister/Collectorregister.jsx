...
if (!licenceRegex.test(data.licenceno)) {
  ...
  function show(e) {
        ...
        // --- DEBUGGING START ---
        // Do NOT use console.log(formData);
        // Use this loop to check values:
        ...
        axios.post("http://localhost:3888/collecterreg", formData)
            .then((response) => {
                alert(response.data.message);
                navigate('/collectorlogin');
            })
            .catch((error) => {
                console.error("Axios Error:", error); // Log the actual error
                if (error.response) {
